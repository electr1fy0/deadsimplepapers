...
function createPaperListItem(paper, index) {
  ...
  const metaParts = [paper.slot.toUpperCase()];
  if (paper.course_code) metaParts.push(paper.course_code);
  ...
  li.innerHTML = `
        ...
        <div class="paper-info">
            <span class="paper-title">${paper.type.toUpperCase()} ${paper.semester_name}</span>
            <span class="paper-meta">${metaString}</span>
        </div>
...
function initializeCommandPalette() {
  ...
  const createCmdKItem = (text) => {
    const li = document.createElement("li");
    li.style.cursor = "pointer";
    li.onclick = () =>
      (window.location.href = `/course.html?course=${encodeURIComponent(text)}`);
    li.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span>${text}</span>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin-left: auto;">
