...
async function initializePapers() {
    ...
    if (courseQuery) {
        if (courseTitle)
            courseTitle.textContent = courseQuery;
        await fetchPapers(courseQuery);
    }
    else {
        if (courseTitle)
            courseTitle.textContent = "All Papers";
        await fetchPapers();
    }
    initializeFilters();
}
...
function renderPapers() {
    ...
    const filtered = allPapers.filter((paper) => {
        const typeMatch = filters.exam_type.length === 0 ||
            filters.exam_type.includes(paper.exam_type.toLowerCase());
        const yearMatch = filters.semester_name.length === 0 ||
            filters.semester_name.includes(paper.semester_name);
        const slotMatch = filters.slot.length === 0 ||
...
function createPaperListItem(paper, index) {
    ...
    li.onclick = () => {
        window.open(`${API_BASE}/download?filename=${encodeURIComponent(paper.filename)}`, "_blank");
    };
    return li;
}
...
function getActiveFilters() {
    ...
    document
        .querySelectorAll(".chip input:checked")
        .forEach((input) => {
        if (input.name === "exam_type")
            filters.exam_type.push(input.value);
        if (input.name === "semester_name")
            filters.semester_name.push(input.value);
        if (input.name === "slot")
            filters.slot.push(input.value.toLowerCase());
    });
    return filters;
}
function updateFilterCounts() {
    ...
    ["fat", "cat2", "cat1"].forEach((type) => {
        const count = countMatches((p) => {
            const yearMatch = active.semester_name.length === 0 ||
                active.semester_name.includes(p.semester_name);
            const slotMatch = active.slot.length === 0 || active.slot.includes(p.slot.toLowerCase());
    ...
    ["2026", "2025", "2024", "2023", "2022"].forEach((year) => {
        const count = countMatches((p) => {
            const typeMatch = active.exam_type.length === 0 ||
...
function initializeCommandPalette() {
    ...
    const renderResults = (query = "") => {
        ...
        const filtered = query
            ? courses.filter((c) => c.course_title.toLowerCase().includes(query.toLowerCase()) ||
                (c.course_code &&
    ...
    const createCmdKItem = (course) => {
        const li = document.createElement("li");
        li.style.cursor = "pointer";
        li.onclick = () => navigate(`/course?course_title=${encodeURIComponent(course.course_title)}`);
